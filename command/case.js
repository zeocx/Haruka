...
//greetings
				haruka.on('group-participants-update', async(anu) => {
					try {
						...
						if (anu.action == 'add') {
							await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Welcome Message By ${ownername}`,ppmem2, [{"buttonId": `.owner`,"buttonText": {"displayText": "welcome cok"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await haruka.sendMessage(from, {
					...
					haruka.sendMessage(from, `Nih gan kontak owner gw, Gausah di chat lagi coli 🤝`, text, {quoted: hehe})
				}
			break
